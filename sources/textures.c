...
/*   Updated: 2024/11/19 10:22:46 by shaintha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_textures(t_cube *cube, t_ray *rays, int x, int y)
{
	...
	{
		tex_y = (int)tex_pos & (cube->image_h - 1);
		tex_pos += step;
			...
			int *tex_x, double *step)
{
	...
	*tex_x = rays->wall_hit * cube->image_w;
	if ((rays->side == 0 && rays->dir_x < 0)
		|| (rays->side == 1 && rays->dir_y > 0))
		*tex_x = cube->image_w - *tex_x - 1;
	*step = cube->image_h * 1.0 / rays->line_height;
...
int	get_texture_color(t_cube *cube, t_ray *rays, int tex_x, int tex_y)
{
	...
	{
		if (rays->dir_y < 0)
			tex_color = cube->wall_south.addr[tex_x + cube->image_h * tex_y];
		else
			tex_color = cube->wall_north.addr[tex_x + cube->image_h * tex_y];
	}
	else
	{
		if (rays->dir_x < 0)
			tex_color = cube->wall_east.addr[tex_x + cube->image_h * tex_y];
		else
			tex_color = cube->wall_west.addr[tex_x + cube->image_h * tex_y];
	}
	if (rays->side == 1)
