...
void	calculate_distances(t_player *player, t_rays *rays)
{
...
void	calculate_wall_height(t_player *player, t_rays *rays)
{
	...
	if (rays->side == 0)
		wall_dist = (rays->map_x - player->pos_x + (float)(1 - rays->step_x) / 2) / rays->dir_x;
	else
		wall_dist = (rays->map_y - player->pos_y + (float)(1 - rays->step_y) / 2) / rays->dir_y;
	...
	if (rays->side == 0)
		rays->wall_hit = player->pos_y + wall_dist * rays->dir_y;
	 	...
	 	rays->wall_hit = player->pos_x + wall_dist * rays->dir_x;
